/**
 * Technical documentation regarding the user story crm00_01: contact edition
 * <br/>
 * <br/>
 *
 * <h2>1. Requirement</h2>
 ...
 * also have a calendar with events (that may appear in a different sidebar).
 * One of the contacts can be the user with an active session on the computer.
 * For the cleansheets user when it reaches the date of an event it should
 * appear a window that notifies. An event has just a "timestamp" of its
 * occurence and descriptive text.
 * <br/>
 * <br/>
 * <b>Use Case "Contact Edition":</b> The user selects one of the option:
 * Create, Edit and Remove. If the user choose option create it will appear a
 * window for the user to enter his first and last name and to select an image
 * or a photo for the contact. On the otherhand, if the choosen option is edit
 * the user has the possibility to change all the information associated with
 * the contact. Otherwise if the user opted for the removing option it will
 * appear all contacts that the user can remove in order to select. The system
 * will validate the information and ask the user for confirmation. The system
 * informs the sucess of the use case. <br/>
 * <br/>
 *
 *
 * <h2>2. Analysis</h2>
...
package csheets.userstories.crm01_01.contact_edition.i130616;
