...
public class NavigationFunctionController {
	...
	public List<String> fillListOperators(List<String> list) {
		try {
			list.add(Language.getInstance().getBinaryOperator("+").
				getIdentifier());
			list.add(Language.getInstance().getBinaryOperator("*").
				getIdentifier());
			list.add(Language.getInstance().getBinaryOperator("-").
			...
			Logger.getLogger(NavigationFunctionController.class.getName()).
	...
	public Value callBinaryOperation(String item, String parameter1,
									 String parameter2) throws IllegalValueTypeException, ParseException {
		Value value = new Value();
		try {
			BinaryOperator operator = Language.getInstance().
				getBinaryOperator(item);
			Literal leftOperand = new Literal(Value.
				parseNumericValue(parameter1));
			Literal rightOperand = new Literal(Value.
				parseNumericValue(parameter2));
